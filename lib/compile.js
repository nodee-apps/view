...
/**
 * extract localized texts like @L("my label") or @L('my label') - quotes can be escaped like @L(" escaped \" ")
 * @param {String} htmlString
 * @returns {Object} locals
 */
function extractLocals(htmlString){
    var locals = {}, texts, localText;
    texts = (htmlString || '').match(/(@L\("(?:[^"\\]|\\.)*"\))/g) || [];
    texts = texts.concat((htmlString || '').match(/(@L\('(?:[^'\\]|\\.)*'\))/g) || []);
    texts = texts.concat((htmlString || '').match(/(@L\(&quot;(?:[^&\\]|\\.)*&quot;\))/g) || []);
    texts = texts.concat((htmlString || '').match(/(@L\(&apos;(?:[^&\\]|\\.)*&apos;\))/g) || []);
...
function evaluate(viewString){
    ...
    for(var lkey in extractedLocals) {
        result = result.replace('@L("' + lkey + '")', '+locals["' +lkey+ '"]+')
                       .replace('@L(\'' + lkey + '\')', '+locals["' +lkey+ '"]+');
    }
    
    var fnc = eval( '(function(model, mode, body, partials, containers, widgetId, dynamicKeys, locals){'+
                        'partials = partials || {};'+
                        'containers = containers || {};'+
                        'widgetId = widgetId || "";'+

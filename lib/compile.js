...
function insertFunctionsToHtml(htmlString, mapping, cheerioOpts, templateId){
    ...
    var containers = [];
    $('[e-container],[e-container-id],[ne-container],[ne-container-id]').each(function(){
        var cnt = $(this);
        var containerId = cnt.attr('e-container') || cnt.attr('e-container-id') || cnt.attr('ne-container') || cnt.attr('ne-container-id');
        
        var allowedWidgets = (cnt.attr('e-container-templates') || cnt.attr('e-container-widgets') || cnt.attr('ne-container-templates') || cnt.attr('ne-container-widgets') || '').replace(/\s/g,'').split(',');
        // clean all empty ids
    ...
    $('[hide-if-mode]').each(function(){
        var elm = $(this);
        var modeValue = elm.attr('hide-if-mode');
        elm.removeAttr('hide-if-mode');
        var content = $.html(elm).replace(/\n(\s|\n)*\n/g, '\n');

...
var CODE_START = '##!',
    ...
    QUOTE = '~!!~',
    QUOTE_REGEX = /\~!!\~/g,
    TEMPLATE_ID_ATTRIBUTE = 'ne-template-id',
    SHOW_TEMPLATE_ID_MODE = 'admin', // if mode=SHOW_TEMPLATE_ID_MODE, show ne-template-id="..." attribute
    WIDGET_ID_ATTRIBUTE = 'ne-widget-id';
...
function insertFunctionsToHtml(htmlString, mapping, cheerioOpts, templateId){
    ...
    function insertRepeatMap(mapping, elms) {
        ...
        var html = '<ne-render-code>'+CODE_START+
                        '(function(refArray){' +
    ...
    $('partial,widget').each(function(){
        ...
        if(partialName) {
            ...
            self.replaceWith('<ne-render-code>'+CODE_START+'partials[' +(partials.length-1)+ ']'+CODE_END+'</ne-render-code>');
        }
    });
    
    // insert widget containers content placeholders
    var containers = [];
    $('[ne-container],[ne-container-id]').each(function(){
        var cnt = $(this);
        var containerId = cnt.attr('ne-container') || cnt.attr('ne-container-id');
        
        var allowedWidgets = (cnt.attr('ne-container-templates') || cnt.attr('ne-container-widgets') || '').replace(/\s/g,'').split(',');
        // clean all empty ids
        ...
        if(containerId) {
            ...
            cnt.empty();
            cnt.append('<ne-render-code>'+CODE_START+'(containers[' +APOS+containerId+APOS+ ']||'+APOS+APOS+')'+CODE_END+'</ne-render-code>');
            
            // remove ne-container, ne-container-widgets attributes
            cnt
            .removeAttr('ne-container')
            .removeAttr('ne-container-id')
            .removeAttr('ne-container-widgets')
            .removeAttr('ne-container-templates');
            
            // disable showing container id attributes
            cnt.attr('ne-render-attribute', CODE_START+
                     '(mode===' +APOS+SHOW_TEMPLATE_ID_MODE+APOS+ ' ? ' +
                     APOS + 'ne-container='+QUOTE+containerId+QUOTE +
                     ' ne-container-widgets='+QUOTE+allowedWidgets+QUOTE+APOS + ':'+APOS+APOS+')'+CODE_END);
        }
    });
    ...
    if(body.length === 1)
        ...
        body.first().replaceWith('<ne-render-code>'+CODE_START+'body'+CODE_END+'</ne-render-code>');
    ...
    if(rootChildsElms) {
        rootChildsElms.each(function(){
            var self = $(this);
            var oldValue = self.attr('ne-render-attribute') || '';
            ...
            self.attr('ne-render-attribute', newValue);
        });
    }
    
    // insert show if mode function
    $('[ne-show-if-mode]').each(function(){
        var elm = $(this);
        var modeValue = elm.attr('ne-show-if-mode');
        elm.removeAttr('ne-show-if-mode');
        var content = $.html(elm).replace(/\n(\s|\n)*\n/g, '\n');
        
        elm.replaceWith('<ne-render-code>'+CODE_START+
                        '(mode===' +APOS+modeValue+APOS+ ' ? ' +
                        APOS + content + APOS + ' :' + APOS+APOS+')'+
                        CODE_END+'</ne-render-code>');
    });
    
    // insert hide if mode function
    $('[ne-hide-if-mode]').each(function(){
        var elm = $(this);
        var modeValue = elm.attr('ne-hide-if-mode');
        elm.removeAttr('ne-hide-if-mode');
        var content = $.html(elm).replace(/\n(\s|\n)*\n/g, '\n');
...
/**
 * extract localized texts like @T("my label") or @T('my label') - quotes can be escaped like @T(" escaped \" ")
 * @param {String} htmlString
 * @returns {Object} locals
 */
function extractLocals(htmlString){
    var locals = {}, texts, localText;
    texts = (htmlString || '').match(/(@T\("(?:[^"\\]|\\.)*"\))/g) || [];
    texts = texts.concat((htmlString || '').match(/(@T\('(?:[^'\\]|\\.)*'\))/g) || []);
    texts = texts.concat((htmlString || '').match(/(@T\(&quot;(?:[^&\\]|\\.)*&quot;\))/g) || []);
    texts = texts.concat((htmlString || '').match(/(@T\(&apos;(?:[^&\\]|\\.)*&apos;\))/g) || []);

    for(var i=0;i<texts.length;i++){
        if(texts[i][3] === '&') texts[i] = texts[i].replace(/&apos;/g,'"').replace(/&quot;/g,'\'');
        localText = texts[i].substring(4, texts[i].length-2 )
                            .replace(/\\"/g,'"')
                            .replace(/\\'/g,'\'');
        locals[ localText ] = localText;
    }
    return locals;
}
...
function evaluate(viewString){
    ...
    viewString = viewString
        ...
        .replace(LT_REGEX,'<')
        .replace(GT_REGEX,'>')
        .replace(/<ne-render-code([^>]+)>/g,'')
        .replace(/<ne-render-code>/g,'')
        .replace(/<\/ne-render-code>/g,'')
        .replace(/\r\n/g, '\n') // use unix, single char newline
        .replace(/\r/g, '\n') // use unix, single char newline
        .replace(/\'/g, '\\\'')
        .replace(/ne-render-attribute="([^"]*)"/g,'$1') // remove attribute placeholder
        .replace(QUOTE_REGEX, '"');
    ...
    // extract localizations from result string
    var extractedLocals = extractLocals(result);
    
    // and replace with property getters
    for(var lkey in extractedLocals) {
        result = result.replace('@T("' + lkey + '")', '\'+(locals[\'' +lkey+ '\']||\''+lkey+'\')+\'')
                       .replace('@T(\'' + lkey + '\')', '\'+(locals[\'' +lkey+ '\']||\''+lkey+'\')+\'')
                       .replace('@T(&quot;' + lkey + '&quot;)', '\'+(locals[\'' +lkey+ '\']||\''+lkey+'\')+\'')
                       .replace('@T(&apos;' + lkey + '&apos;)', '\'+(locals[\'' +lkey+ '\']||\''+lkey+'\')+\'');
    }
    
    var fnc = eval( '(function(model, locals, mode, body, partials, containers, widgetId, dynamicKeys){'+
                        'partials = partials || {};'+
                        'containers = containers || {};'+
                        'widgetId = widgetId || "";'+
                        'locals = locals || extractedLocals;'+
                        'return ' +result+ ';})');
    
    fnc.locals = extractedLocals;
    return fnc;
}

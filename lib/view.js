...
/**
 * script file names - only for temporary usage, in case when scipt file is created before tamplate file is compiled
 * @type {Object}
 */
var view_scripts = {};
            ...
            if(file.isFile){
                ...
                if(file.ext === VIEW_FILE_EXT){
                    if(event === 'removed') delete views[ viewId ];
                        ...
                        compileAsync(file.fullPath, file.fullPath + '.' + MAP_FILE_EXT, viewId, function(err, viewFnc){
                            if(err) views[ viewId ] = err;
                                ...
                                var script = views[ viewId ].script || view_scripts[ viewId ]; // copy script reference
                                delete view_scripts[ viewId ]; // no need to cache script file reference in temp object
                    ...
                    if(event === 'removed'){
                        ...
                        delete view_scripts[ viewId ];
                    }
                    if(event === 'updated'){
                        delete require.cache[ require.resolve(file.fullPath) ];
                    }
                    else if(event === 'created') {
                        if(views[ viewId ]) views[ viewId ].script = file.fullPath;
                        else view_scripts[ viewId ] = file.fullPath;
                    }
                }
            }
        })(i);
        ...
        fs.exists(mapFilePath, function(exists){
            if(exists) fsExt.readFile(mapFilePath, { encoding:'utf8' }, function(err, data){
                if(err) cb(new Error('View engine compileAsync: reading mapping file failed').cause(err));
                    ...
                    if(Object.prototype.toString.call(mapping) !== '[object Object]') return cb(new Error('View engine compileAsync: mapping is not object').details({ template:viewFilePath }));
 ...
 * @returns {String}  html
 */
function renderView(baseDirId, viewName, model, locals, mode, containers, replaceViewNameFnc){
    ...
    var result = pageViewFnc(model, locals, mode, '', renderPartials(baseDirId, model, locals, mode, pageViewFnc.partials, containers, replaceViewNameFnc), containers);
    ...
    if(pageViewFnc.layout){
        ...
        result = layoutViewFnc(model, locals, mode, result, renderPartials(baseDirId, model, locals, mode, layoutViewFnc.partials, containers, replaceViewNameFnc), containers);
    }
    return result;
}
 ...
 */
function renderPartials(baseDirId, model, locals, mode, partials, containers, replaceViewNameFnc){
    var result = [];
    if(!partials || partials.length===0) return [];
        ...
        for(var i=0;i<partials.length;i++){
            ...
            result.push(partialViewFnc(model, locals, mode, '', [], containers||{}, partials[i].id, dynamicKeys));
        }
    }
    return result;
}
 ...
 * @returns {String}  html
 */
function renderViewSync(baseDirId, viewName, model, locals, mode, containers, replaceViewNameFnc){
    var pageViewFnc = getCompiledSync(baseDirId, viewName, replaceViewNameFnc);
    var result = pageViewFnc(model, locals, mode, '', renderPartialsSync(baseDirId, model, locals, mode, pageViewFnc.partials, containers, replaceViewNameFnc), containers||{});
    
    if(pageViewFnc.layout){
        var layoutViewFnc = getCompiledSync(baseDirId, pageViewFnc.layout, replaceViewNameFnc);
        result = layoutViewFnc(model, locals, mode, result, renderPartialsSync(baseDirId, model, locals, mode, layoutViewFnc.partials, containers, replaceViewNameFnc), containers||{});
 ...
 * @returns {Array}  partials content html
 */
function renderPartialsSync(baseDirId, model, locals, mode, partials, containers, replaceViewNameFnc){
    var result = [];
    if(!partials || partials.length===0) return [];
    else {
        for(var i=0;i<partials.length;i++){
